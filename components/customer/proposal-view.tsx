...
export function ProposalView({ initialProposal }: ProposalViewProps) {
    ...
    const handleDecision = async (decision: "ACCEPTED" | "REFUSED") => {
        setIsSubmitting(true);
        try {
            const body: any = { decision };
            
            if (decision === "ACCEPTED") {
                body.deliveryAddress = proposal.deliveryAddress || "Adresse à confirmer";
                body.paymentChoice = proposal.paymentChoice || "PAY_ON_DELIVERY";
            }
            ...
            <div className="w-full max-w-md space-y-6">
                ...
                {isExpired ? (
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            {isSubmitting ? "Traitement..." : "✓ Accepter la Livraison"}
                        </button>
                        <button 
                            ...
                            disabled={isSubmitting}
                        >
                            {isSubmitting ? "Traitement..." : "✗ Refuser (Retrait en agence)"}
                        </button>
                    </div>
                ) : (
